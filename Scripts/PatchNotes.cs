...
public class PatchNotes : SerializedScriptableObject
{
    ...
    public bool IsLatestRead()
    {
        ...
        if (!ES3.KeyExists(GetLatestVersion().ToString() + " has been read", "patchnotes.es3"))
            return false;
    ...
    {
#if UNITY_EDITOR
        if (patchNotes.TryGetValue(version, out NoteContainer currentNotes) && currentNotes.notes == notes)
