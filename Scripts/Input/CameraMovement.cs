...
public class CameraMovement : MonoBehaviour
{
    ...
    {
        cameraActions.LoadBindingOverridesFromJson(rebinds);
        movement = cameraActions.Camera.Movement;
    }

    private void TechTreeOpen(bool techTreeOpen)
    {
        allowCameraMovement = !techTreeOpen;
    }
    ...
    {
        Hex3 targetPosition = (transform.position + this.targetPosition).ToHex3();
        int distance = Mathf.Abs(targetPosition.Max());

        return distance < this.hexLimit;
    }

    private void MoveToObject(Vector3 position)
    {
        if (gameObject != null)
            this.transform.DOMove(position, 0.25f);
    }
    ...
    [Button]
    private void SetZoomHeight(float zoomHeight)
    {
    ...
    private IEnumerator Zooming()
    {
        ...
        zoomDampening = 0.75f;
        this.zoomHeight = 12f;
        yield return new WaitUntil(() => Mathf.Abs(cameraTransform.position.y - zoomHeight) < 0.5f);
        cameraActions.Camera.Enable();
        yield return new WaitUntil(() => Mathf.Abs(cameraTransform.position.y - zoomHeight) < 0.1f);
        zoomDampening = 7.5f;
